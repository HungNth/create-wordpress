...
// Directories excluded from the backup zip are configured in config.json
// under the key "backup_excludes". Edit that file to customise.

function addFolderToZip(zip, sourceDir, zipRoot = '', excludes = new Set()) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			addFolderToZip(zip, fullPath, zipPath, excludes);
...
async function backupFullSource(siteName, siteDir, backupsDir, config) {
	const excludes = new Set(
		Array.isArray(config.backup_excludes)
			? config.backup_excludes
			: ['.idea', '.vscode', 'node_modules', '__MACOSX']
	);
	...
	try {
		...
		addFolderToZip(zip, siteDir, '', excludes);
