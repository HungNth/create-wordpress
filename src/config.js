...
const DEFAULT_WP_TWEAKS = [
	...
	{ type: 'option_update', key: 'default_pingback_flag', value: '0' },
...
function migrateConfig(config) {
	...
	if (typeof config.use_herd !== 'boolean') {
		config.use_herd = true;
		changed = true;
	}
...
async function createConfig() {
	...
	const answers = await inquirer.prompt([
		{
			type: 'confirm',
			name: 'use_herd',
			message: 'Do you use Laravel Herd?',
			default: true,
		},
		{
			type: 'input',
	...
	const config = {
		use_herd: answers.use_herd !== false,
